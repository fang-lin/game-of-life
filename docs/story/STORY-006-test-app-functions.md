...
- [x] `parseParams` / `stringifyParams` 测试覆盖：正常值、边界值、非法值
- [x] `parseString` 测试覆盖：默认值回退、超范围值
- [x] `stringifyCoordinate` 测试覆盖：正常数字、NaN、负数
- [x] `rotateCells` 测试覆盖：90°/180°/270° 旋转
- [x] `combinePathToURL` 测试覆盖
- [x] cells 编解码往返测试（encode → decode 一致性）
- [x] 覆盖率 >90%
...
|------|---------|
| 2026-05-04 | Story 创建 |
| 2026-05-04 | 30 unit tests added, all passing |
