...
- [x] Canvas 改为函数组件
- [x] 移除 `LifeMapHooks` 接口和 `setLifeMapHooks` 回调
- [x] LifeMap 实例管理改用 `useRef`
- [x] `requestAnimationFrame` 动画循环使用 `useEffect` + cleanup
- [x] `shouldLayoutCanvas` 逻辑用 `useEffect` 依赖数组替代
- [x] hoveringCells 管理改用 `useState` 或 `useRef`
- [x] 所有现有功能不变
...
|------|---------|
| 2026-05-04 | Story 创建 |
| 2026-05-04 | Canvas absorbed into App as canvas element with ref |
